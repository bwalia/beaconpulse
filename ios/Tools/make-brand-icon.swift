...
//
//   swift ios/Tools/make-brand-icon.swift --hex F59E0B \
//     --out ios/Brands/Uptimely/Assets.xcassets/AppIcon.appiconset/icon-1024.png
//
...
//
// The mark drawn is Uptimely's — a rising curve breaking upward past a baseline,
// matching frontend/src/brand/uptimely.ts so the phone icon and the web logo are the
// same drawing. A new brand adds its own draw function beside `drawUptimelyMark`.
...
let markMinX: CGFloat = 3.5 - pad, markMaxX: CGFloat = 20.5 + pad
let markMinY: CGFloat = 7.0 - pad, markMaxY: CGFloat = 20.5 + pad
...
// Flip Y so the SVG coordinates from uptimely.ts can be transcribed literally.
ctx.translateBy(x: half - midX * scale, y: half + midY * scale)
ctx.scaleBy(x: scale, y: -scale)

func drawUptimelyMark(_ ctx: CGContext) {
    ctx.setStrokeColor(red: 1, green: 1, blue: 1, alpha: 1)
    ctx.setLineWidth(strokeWidth)
    ctx.setLineCap(.round)
    ctx.setLineJoin(.round)

    // The climb: M3.5 17.5 L9 12 L12.5 15.5 L20.5 7
    ctx.beginPath()
    ctx.move(to: CGPoint(x: 3.5, y: 17.5))
    ctx.addLine(to: CGPoint(x: 9, y: 12))
    ctx.addLine(to: CGPoint(x: 12.5, y: 15.5))
    ctx.addLine(to: CGPoint(x: 20.5, y: 7))
    ctx.strokePath()

    // Its arrowhead: M15.5 7 h5 v5
    ctx.beginPath()
    ctx.move(to: CGPoint(x: 15.5, y: 7))
    ctx.addLine(to: CGPoint(x: 20.5, y: 7))
    ctx.addLine(to: CGPoint(x: 20.5, y: 12))
    ctx.strokePath()

    // The baseline it stays above: M3.5 20.5 h17
    ctx.beginPath()
    ctx.move(to: CGPoint(x: 3.5, y: 20.5))
    ctx.addLine(to: CGPoint(x: 20.5, y: 20.5))
    ctx.strokePath()
}

drawUptimelyMark(ctx)
