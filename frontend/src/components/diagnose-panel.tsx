...
// labelKey is a literal message key resolved with t() at render, so the confidence
// wording is translated rather than baked into this module.
const CONFIDENCE: Record<
  string,
  { labelKey: "confidenceHigh" | "confidenceMedium" | "confidenceLow"; className: string }
> = {
  high: {
    labelKey: "confidenceHigh",
    className: "bg-emerald-100 text-emerald-900 dark:bg-emerald-900/40 dark:text-emerald-200",
  },
  medium: {
    labelKey: "confidenceMedium",
    className: "bg-amber-100 text-amber-900 dark:bg-amber-900/40 dark:text-amber-200",
  },
  low: {
    labelKey: "confidenceLow",
        ...
        {analysis ? (
          <>
            ...
            <dl className="mt-3 space-y-3 text-sm">
              <div>
                <dt className="font-medium text-slate-700 dark:text-slate-200">{t("likelyCause")}</dt>
                <dd className="mt-0.5 text-slate-600 dark:text-slate-300">{analysis.likely_cause}</dd>
              </div>
              <div>
                <dt className="font-medium text-slate-700 dark:text-slate-200">{t("suggestedFix")}</dt>
        ...
        <details className="mt-4 group">
          ...
          <div className="mt-2 rounded-lg bg-slate-50 px-4 py-2 dark:bg-slate-900/60">
            <Section title="DNS" ok={e.dns.resolved}>
              <Row label={t("fieldResolved")} value={String(e.dns.resolved)} bad={!e.dns.resolved} />
              {e.dns.addresses?.length ? <Row label={t("fieldAddresses")} value={e.dns.addresses.join(", ")} /> : null}
              {e.dns.cname ? <Row label={t("fieldCname")} value={e.dns.cname} /> : null}
              {e.dns.nameservers?.length ? <Row label={t("fieldNameservers")} value={e.dns.nameservers.join(", ")} /> : null}
              <Row label={t("fieldLookup")} value={`${e.dns.lookup_ms} ms`} />
              {e.dns.error ? <Row label={t("fieldError")} value={e.dns.error} bad /> : null}
            </Section>

            <Section title="TCP" ok={e.tcp.attempted ? e.tcp.connected : null}>
              {e.tcp.attempted ? (
                <>
                  <Row label={t("fieldConnected")} value={String(e.tcp.connected)} bad={!e.tcp.connected} />
                  {e.tcp.address ? <Row label={t("fieldAddress")} value={e.tcp.address} /> : null}
                  <Row label={t("fieldConnect")} value={`${e.tcp.connect_ms} ms`} />
                  {e.tcp.error ? <Row label={t("fieldError")} value={e.tcp.error} bad /> : null}
                </>
              ) : (
                <Row label="—" value={t("notAttemptedEarlier")} />
              )}
            </Section>
            ...
            <Section title="TLS" ok={e.tls.attempted ? e.tls.handshake_ok && !e.tls.expired && e.tls.hostname_ok : null}>
              {e.tls.attempted ? (
                <>
                  <Row label={t("fieldHandshake")} value={String(e.tls.handshake_ok)} bad={!e.tls.handshake_ok} />
                  {e.tls.issuer ? <Row label={t("fieldIssuer")} value={e.tls.issuer} /> : null}
                  {e.tls.subject ? <Row label={t("fieldSubject")} value={e.tls.subject} /> : null}
                  {e.tls.not_after ? (
                    <Row
                      label={t("fieldExpires")}
                      value={t("fieldExpiresValue", {
                        notAfter: e.tls.not_after,
                        days: e.tls.days_remaining ?? 0,
                      })}
                      bad={e.tls.expired || (e.tls.days_remaining ?? 99) < 14}
                    />
                  ) : null}
                  {e.tls.attempted && !e.tls.hostname_ok ? (
                    <Row label={t("fieldHostname")} value={t("certNoHostname")} bad />
                  ) : null}
                  {e.tls.error ? <Row label={t("fieldError")} value={e.tls.error} bad /> : null}
                </>
              ) : (
                <Row label="—" value={t("notAttemptedTls")} />
              )}
            </Section>
            ...
            <Section title="HTTP" ok={e.http.attempted ? (e.http.status_code ?? 0) < 400 : null}>
              {e.http.attempted ? (
                <>
                  {e.http.status_code ? (
                    <Row label={t("fieldStatus")} value={String(e.http.status_code)} bad={e.http.status_code >= 400} />
                  ) : null}
                  <Row label={t("fieldResponse")} value={`${e.http.response_ms} ms`} />
                  {e.http.redirect_chain?.length ? (
                    <Row label={t("fieldRedirects")} value={e.http.redirect_chain.join(" → ")} />
                  ) : null}
                  {e.http.server ? <Row label={t("fieldServer")} value={e.http.server} /> : null}
                  {e.http.error ? <Row label={t("fieldError")} value={e.http.error} bad /> : null}
                </>
              ) : (
                <Row label="—" value={t("notAttemptedHttp")} />
              )}
            </Section>

            <p className="border-t border-slate-200 pt-2 text-[11px] text-slate-400 dark:border-slate-800">
              {t("probed", { time: new Date(e.checked_at).toLocaleString(), target: e.target })}
            </p>
          </div>
        </details>
      </Card>
...
export function DiagnoseUpsell({ onDismiss }: { onDismiss: () => void }) {
  ...
  return (
    <Card className="mt-3 border-brand-200 p-5 dark:border-brand-900">
      <h4 className="font-semibold text-slate-900 dark:text-white">{t("upsellTitle")}</h4>
      <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{t("upsellBody")}</p>
      <div className="mt-4 flex gap-2">
        <Button onClick={() => (window.location.href = "/billing")}>{t("addCredit")}</Button>
...
export function useDiagnoseControl(monitorId: string) {
  ...
  return { run, isPending: diagnose.isPending, panel, label: diagnoseLabel(billing, diagnose.isPending, t) };
}
...
function DiagnoseCost({ info }: { info: BillingInfo }) {
  ...
  if (info.effective_plan === "payg") {
    ...
    return (
      <div className="space-y-2">
        <p>{t.rich("costPaygUses", { minutes: costMinutes, b: bold })}</p>
        <p className="text-sm text-slate-500 dark:text-slate-400">
          {t("costPaygLeft", { minutes: creditMinutes, enough: enough ? "yes" : "no" })}
        </p>
        <p className="text-sm text-slate-500 dark:text-slate-400">{t("costPaygRefund")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      <p>{t.rich("costSubUses", { total: info.monthly_diagnoses, b: bold })}</p>
      <p className="text-sm text-slate-500 dark:text-slate-400">
        {t("costSubLeft", { used: info.diagnoses_used_this_month, left })}
      </p>
      <p className="text-sm text-slate-500 dark:text-slate-400">{t("costSubRefund")}</p>
    </div>
  );
}
 ...
 * you find out.
 */
export function diagnoseLabel(
  info: BillingInfo | undefined,
  pending: boolean,
  t: ReturnType<typeof useTranslations<"diagnose">>,
): string {
  if (pending) return t("diagnosing");
  if (!info || info.effective_plan === "free") return t("diagnoseWithAi");
  if (info.effective_plan === "payg") {
    return t("diagnoseWithAiCredit", { min: Math.round(info.diagnosis_cost_seconds / 60) });
  }
  const left = Math.max(0, info.monthly_diagnoses - info.diagnoses_used_this_month);
  return t("diagnoseWithAiLeft", { left });
}
